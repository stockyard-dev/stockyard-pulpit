...
async function load(){var r=await fetch(A+'/broadcasts').then(function(r){return r.json()});items=r.broadcasts||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.title||'').toLowerCase().includes(q)||(i.body||'').toLowerCase().includes(q)||(i.audience||'').toLowerCase().includes(q)||(i.channel||'').toLowerCase().includes(q)});
...
if(i.audience)h+='<span>'+esc(i.audience)+'</span>';
if(i.channel)h+='<span>'+esc(i.channel)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.scheduled_at)h+='<span>'+esc(i.scheduled_at)+'</span>';
if(i.sent_at)h+='<span>'+esc(i.sent_at)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/broadcasts/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{title:"",body:"",audience:"",channel:"",status:"",scheduled_at:"",sent_at:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' PULPIT</h2>';
...
h+='<div class="fr"><label>Audience</label><input id="f-audience" value="'+esc(i.audience)+'"></div>';
...
h+='<div class="fr"><label>Scheduled At</label><input id="f-scheduled-at" value="'+esc(i.scheduled_at)+'"></div>';
h+='<div class="fr"><label>Sent At</label><input id="f-sent-at" type="date" value="'+esc(i.sent_at)+'"></div>';
...
async function submit(){var body={title:document.getElementById('f-title').value.trim(),body:document.getElementById('f-body').value.trim(),audience:document.getElementById('f-audience').value.trim(),channel:document.getElementById('f-channel').value.trim(),status:document.getElementById('f-status').value.trim(),scheduled_at:document.getElementById('f-scheduled-at').value.trim(),sent_at:document.getElementById('f-sent-at').value.trim()};
if(editId){await fetch(A+'/broadcasts/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/broadcasts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
